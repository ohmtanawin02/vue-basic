...
const validate = {
  required: (val, name) => {
    ...
    return name ? `please enter: ${name}` : 'this field is required'
  },
  ...
  length: (val, length) =>
    (!!val && val.length === length) || `Input should be ${length} letter(s)`,
  minLength: (val, length) =>
    (!!val && val.length >= length) || `Please enter at lease ${length} letter(s)`,
  maxLength: (val, length) =>
    (!!val && val.length <= length) || `Please enter at most ${length} letter(s)`,
  between: (val, min, max) =>
    (!!val && val.length >= min && val.length <= max) ||
    `please enter between ${min} and ${max} letter`,
  valueBetween: (val, min, max) =>
    (!!val && +val >= min && +val <= max) || `please enter between ${min} and ${max}`,
  atLeast: (val, length = 1) =>
    (!!val.length && val.length >= length) || `Please select at least ${length} option(s)`,
  numWithDecimal: (val) => /^\d+(\.\d+)*$/.test(val) || 'Please enter only number and decimal',
  numCommaDecimal: (val) =>
    /^\d+(,\d{3})*(\.\d+)?$/.test(val) || 'Please enter only number, comma and decimal',
